...
shared_geography_params = [
    ...
    ('state:11', None),
    ('county:610', 'state:51'),
    ('county subdivision:28360', 'state:26+county:115'),
    ...
    ('new england city and town area:71650', 'combined new england city and town area:715'),
...
#!    ('county (or part):015', 'state:33+state legislative district (lower chamber):726'),
    ('public use microdata area:03803', 'state:36'), #ACS
#!!    ('zip code tabulation area:82073', 'state:*'), # 'state' not a variable in tiger for ZCTAs
#    ('school district (elementary):32340', 'state:06'),
#    ('school district (secondary):17903', 'state:17'),
#    ('school district (unified):00780', 'state:34'),
]
...
@pytest.mark.parametrize('for_string, in_string', shared_geography_params + acs_geography_params)
def test_all_acs_geographies(ACSProductApi, for_string, in_string):
    if '(or part)' in for_string:
        assert False
...
@pytest.mark.parametrize('for_string, in_string', shared_geography_params + dec_geography_params)
def test_all_dec_geographies(DecennialProductApi, for_string, in_string):
